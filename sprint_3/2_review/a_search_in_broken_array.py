...
будет, в таком случае поиск вернёт -1. Чтобы найти разрыв, определим индекс
центрального элемента, для этого сложим левый и правый края, в начале они будут
равны 0 и n-1, соответственно, где n — длинна массива, сумму поделим нацело на
2. Проверим, если слева от центрального элемента есть ещё другие элементы, то
не является ли предыдущий элемент от центрального разрывом. Таким же образом,
проверим, если справа от центрального элемента есть ещё элементы, то возможно
центральный элемент — это разрыв. Если разрыв не будет найден, то у нас
остаётся две половинки, одна из половинок сломана, в ней и находится разрыв.
Определим сломанность по границам половинок, если левая граница больше правой,
то данная половинка сломанна, в ней и продолжим поиск. Таким образом, если
разрыв не найден, мы просто запускаем бинарный поиск по массиву, иначе мы
проверяем, что разрыв это искомый элемент. Если это не так, мы определяем
какому из двух отрезков принадлежит искомый элемент, по его границам, и
запускаем бинарный поиск внутри этого отрезка.
