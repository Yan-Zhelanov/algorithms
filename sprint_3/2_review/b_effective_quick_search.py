...
Нам придётся хранить индексы двух границ, двух краёв и индекс опорного элемента
для каждого рекурсивного вызова. Таких вызовов одновременно будет в лучшем
случае log(n), так как сначала будут обрабатываться вызовы для левых частей,
а после для правых. В худшем случае, таких вызовов будет n, по той же причине.
В итоге, в лучшем и среднем случае мы потратим: O(log(n) * 5) = O(log(n))
В худшем: O(n*5) = O(n)
"""
