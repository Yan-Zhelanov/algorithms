...
def compare(num_1, num_2):
    index_1 = index_2 = 0
    while (
        num_1[index_1] == num_2[index_2]
        and (
            index_1 < len(num_1) - 1
            or index_2 < len(num_2) - 1
        )
    ):
        if index_1 < len(num_1) - 1:
            index_1 += 1
        if index_2 < len(num_2) - 1:
            index_2 += 1
    if index_1 < index_2 or num_1[index_1] > num_2[index_2]:
        return -1
    elif index_1 > index_2 or num_1[index_1] < num_2[index_2]:
        return 1
    else:
        return 0
...
def test_determine_biggest_number():
    ...
    result = determine_biggest_number(['825', '823', '8', '8', '828', '825', '828', '828', '82'])
    assert result == '8882828828828825825823', f'Wrong answer: {result}'
